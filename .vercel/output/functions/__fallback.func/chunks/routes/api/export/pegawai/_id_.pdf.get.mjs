import { d as defineEventHandler, l as getRouterParam, c as createError, q as setHeader } from '../../../../_/nitro.mjs';
...
const _id__pdf_get = defineEventHandler(async (event) => {
  var _a;
  const path = event.path || "";
  const id = getRouterParam(event, "id") || ((_a = path.match(/\/(\d+)\.pdf$/)) == null ? void 0 : _a[1]);
  ...
  const pendList = pendidikan.map(
    (pd) => `${pd.tingkat_pendidikan || "-"} / ${pd.nama_sekolah || "-"} / ${pd.tahun_lulus || "-"}`
  );
  const docDef = {
    content: [
      ...
      { text: "\nPendidikan:\n" },
      ...pendList.length > 0 ? pendList.map((t) => ({ text: `- ${t}` })) : [{ text: "-" }]
    ],
    styles: {
...
export { _id__pdf_get as default };
//# sourceMappingURL=_id_.pdf.get.mjs.map
